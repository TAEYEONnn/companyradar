...
- `/admin` 운영자 대시보드 접근 방식을 수정했습니다. 이제 클라이언트 컴포넌트 기반으로 동작하며, `dev@example.com` 또는 `AI_ALLOWED_EMAILS` 계정으로 로그인 후 `http://localhost:3000/admin`에서 확인 가능합니다.
- 팔로업 완료 태스크를 "완료됨 N개" 토글로 접어서 숨길 수 있습니다. 기본은 접힌 상태입니다.
- 회사 삭제 시 드로어가 즉시 닫히지 않고, 확인 다이얼로그가 드로어 위에 뜬 뒤 확인하면 닫힙니다.
- 칸반보드 점수에 색상이 적용됩니다 (4.3+ 초록, 3.7+ 파랑, 3.0+ 노랑, 미만 회색).
- 가중치 슬라이더 변경 후 "저장됨 ✓" 인디케이터를 1.5초간 표시합니다.
- "직군 샘플로 초기화" 아이콘 버튼에 "샘플 초기화" 텍스트 레이블을 추가했습니다.
- 후보 검토 공고 원문/메모 플레이스홀더를 자연스러운 문구로 교체했습니다.
