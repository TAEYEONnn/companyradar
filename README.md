...
- **OG 이미지 히어로 배너 리디자인**: 어두운 다크 그라디언트 배경 + 좌측 대형 헤드라인 + 우측 회사 핏 점수 카드로 재구성. 기존 밝은 배경 좌우 분할 레이아웃에서 임팩트 있는 히어로 배너 스타일로 전환.
- **Magic link 에러 dev 힌트 개선**: 비밀번호 재설정 메일 발송 실패 시 `console.error`로 실제 오류를 기록. 개발 환경에서는 오류 텍스트와 Supabase 대시보드 Redirect URLs 등록 안내 메시지를 UI에 인라인 표시.
