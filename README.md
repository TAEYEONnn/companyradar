...
- 드로어 상단 회사 정보가 스크롤하면 자동으로 접히고, 탭과 콘텐츠 영역이 확장됩니다. 스크롤을 올리면 다시 펼쳐집니다.
- 정보 출처 라벨을 "채용공고만 봤어요", "뉴스·후기 찾아봤어요" 등 구어체로 바꿨습니다. "발견 이유" → "관심 갖게 된 계기", "수동 추가" → "직접 추가", "정보 믿을만함" → "정보 출처", "정보 신뢰 Lv.N" → "정보 깊이 N/5" 등 전반적인 라이팅을 자연스럽게 정리했습니다.
- 고급 필터 버튼에 활성화된 필터 수를 뱃지로 표시합니다.
- 문의·환불 API의 서버 설정 문제(`SUPABASE_SERVICE_ROLE_KEY` 미설정)를 일반 500 대신 503으로 분리해 원인 파악이 쉬워졌습니다.
