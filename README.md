...
- 드로어 본문 행간과 섹션 간격을 넓혀 긴 메모와 회사 정보가 덜 빽빽하게 보이도록 조정했습니다.
- 드로어와 설정 화면의 어려운 표현을 `회사 조사`, `정보 믿을만함`, `좋은 점/걱정되는 점`처럼 쉬운 말로 바꿨습니다.
- `공고 확인했어요`, `평가 점수`, `점수 라벨`, `환불 요청`에 작은 도움말을 추가했습니다.
- 문의 접수 실패 시 서버가 보낸 원인 메시지를 토스트로 보여주고, 서버 로그에도 저장 실패 원인을 남깁니다.
- 운영자 계정은 결제 이력이 없어도 환불 요청 테스트를 접수할 수 있고, 요청 metadata에 `operatorTest: true`가 저장됩니다.
