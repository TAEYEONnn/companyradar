...
- **라이팅 대폭 간소화**: 격식체("~습니다", "~해주세요", "~못했습니다") 전면 제거, 긴 설명 축약, 불필요한 부연 제거.
  - OnboardingModal: "내 직군을 선택해주세요" → "어떤 일을 하고 있나요?", 긴 소개 문구 1줄로 축약
  - CompanyForm: AI 버튼 라벨 축약("자동 채우기" → "AI 분석"), "텍스트 직접 붙여넣기" → "텍스트 붙여넣기", 헤더 서브텍스트 제거
  - Toolbar: 검색 placeholder "회사명, 산업군, 제품 설명 검색" → "검색", "Green/Red 신호 있음" → "좋은/걱정 신호"
  - CompanyDetailPanel: 토스트·에러 메시지 전체 축약, 다음 할 일 배너 자연스러운 문장으로, 기한 레이블 간소화
- **AI 공고 분석 도구로 전략적 포지셔닝**: CompanyRadar를 "회사 관리 툴"에서 "공고 붙여넣으면 AI가 판단까지 도와주는 이직 준비 도구"로 재정의. 핵심 흐름을 "공고 붙여넣기 → AI 분석 → 저장"으로 단순화.
- **사이드바 "후보 검토" → "공고 정리"**: 제품의 주된 역할을 반영해 네비게이션 레이블 변경.
- **공고 정리 패널 UX 전면 재구성**: 6개 폼 필드를 단일 대형 텍스트 입력(URL 또는 원문)으로 대체. URL을 붙여넣으면 자동으로 sourceUrl로, 텍스트면 rawText로 분기. 보조 필드(회사명·직무명·발견이유·첫인상)는 "추가 정보 입력" 토글 아래로 숨김.
- **"AI로 정리하기" 통합 버튼**: 저장과 AI 파싱을 하나의 버튼으로 통합. 클릭 즉시 저장 후 AI 파싱 시작. 파싱 없이 저장만 하려면 "저장만 하기" 링크 사용.
- **빈 상태 안내 문구 업데이트**: StartGuide → "아직 저장한 공고가 없어요. 공고를 붙여넣으면 AI가 초안을 정리해요." / 공고 정리 패널 빈 상태 → "공고를 붙여넣으면 AI가 초안을 정리해요."
- **프로모션 토스트 간소화**: "OO을 회사 목록에 추가했습니다." → "회사 목록에 추가했어요."
- **원격 병합 시 샘플 제거**: remote 데이터에 남아있던 구 샘플(최대 18개)을 merge 전에 필터링. 샘플은 로컬 명시 추가만 유효.
- **동기화 토스트 단순화**: "Supabase에서 데이터를 동기화했습니다." / "기기 데이터와 Supabase 데이터를 병합했습니다." 제거 → remote에 실제 사용자 공고가 있을 때만 "저장한 공고가 동기화됐어요." 표시. 원격 실패 시 토스트 없음.
- **예시 데이터 완전 비활성화**: `normalizeSamplesForRole`를 자동 호출하던 경로 5곳 전부 제거 — 초기 로드 4경로(원격 실패·원격 병합·로컬 보유·신규 진입) + 온보딩 완료 분기. 이제 예시는 명시적으로 "예시 추가" 버튼을 눌러야만 나타남.
- **예시 삭제 후 새로고침 재등장 버그 수정**: 삭제 후 localStorage 저장이 자동 효과로 처리되므로 새로고침해도 빈 목록 유지.
- **빈 화면에서 예시 추가 가능**: StartGuide에 "예시 데이터로 둘러보기" 링크 추가 — 예시 삭제 후 다시 추가 가능.
- **직군 변경 시 예시 자동 교체 제거**: 직군 바꿔도 예시가 자동으로 교체되지 않음. 예시를 원하면 직접 추가.
- **텍스트 다이어트**: 공고 후보 화면 헤더·설명 축약, 폼 "(선택)" 레이블 제거, AI 초안 안내 단축, 예시 배너 단축, 민감 메모 설명 압축, 드로어 "다음 할 일 없음" 배너 간소화.
