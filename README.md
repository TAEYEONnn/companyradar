...
- `/admin` 운영자 대시보드를 추가했습니다. 문의·환불·탈퇴 요청을 탭별로 조회하고, 상태를 변경하거나 "답장 이메일 열기"로 바로 이메일 앱을 열 수 있습니다. `AI_ALLOWED_EMAILS` 또는 `profiles.role = owner` 계정만 접근 가능합니다.
- 드로어에 이전/다음 회사 이동 버튼을 추가했습니다. 키보드 좌우 화살표로도 이동 가능합니다.
- 역할 변경 시 "PM/PO 직군으로 변경했어요" 토스트를 표시합니다.
- Candidate Inbox에서 승격된 회사가 ID 대신 이름으로 표시되며 클릭 시 해당 회사 드로어가 열립니다.
- 필터 결과가 없을 때 "필터 초기화"와 "회사 추가" 버튼을 표시합니다.
