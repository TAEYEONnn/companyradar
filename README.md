...
- 드로어와 회사 등록/수정 폼의 평가 기준 문구가 선택한 직군에 맞게 표시됩니다. 저장 구조와 점수 계산 key는 그대로 유지합니다.
- 드로어 요약 화면의 점수 카드와 기록 날짜 영역을 더 작게 정리했고, 날짜는 `yy.mm.dd` 형식으로 표시합니다.
- 드로어의 중복 “확인 필요한 내용” 영역을 제거하고, 확인 필요 뱃지는 상단에서만 보여줍니다.
- 민감메모는 연봉 협상, 처우 조건, 내부자에게 들은 내용, 소개자 정보처럼 공개 목록에 섞이면 곤란한 내용을 항목별로 저장하고 삭제할 수 있습니다.
- 기존 단일 민감메모 데이터는 “이전 민감 메모” 항목으로 계속 확인할 수 있습니다.
