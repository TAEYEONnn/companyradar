...
- 로컬 환경(devTools 활성, 미로그인)에서 회사 추가·후보 검토 추가가 정상 동작합니다. `effectiveUserId`를 저장/삭제/생성 전반에 적용했습니다.
- 어드민 대시보드에 "앱으로 돌아가기" 버튼을 추가했습니다.
- 어드민 답장 이메일 body에 `companyradar.app@gmail.com` 서명이 자동으로 포함됩니다. 브랜딩이 `[CompanyRadar]`로 통일됩니다.
- 드로어 탭 이름을 개선했습니다: "민감 메모" → "비공개 메모", "AI" → "AI 분석".
- 후보 검토 문구를 개선했습니다: "회사로 승격" → "회사 목록에 추가", "승격됨" → "추가됨", "검토 필요" → "확인 필요".
- 오늘 할 일 패널 섹션 "나머지" → "이후 예정", "처리하기" → "확인하기".
