...
- `dev@example.com`은 로그인/새로고침마다 온보딩을 다시 볼 수 있고, 일반 사용자는 직군 설정이 없을 때만 필수 온보딩을 진행합니다.
- 오늘 할 일과 타임라인에서 회사를 열면 면접/지원 준비/요약 등 상황에 맞는 드로어 탭으로 바로 이동합니다.
- 드로어 헤더를 상단 액션바와 회사 정보 영역으로 나누고, 기록 날짜는 작은 상태줄로 옮겼습니다. 날짜는 `yy.mm.dd` 형식으로 표시됩니다.
- 완료한 일 목록은 기본 접힘 상태이며, 펼친 뒤 다시 열기, 회사 보기, 삭제를 사용할 수 있습니다.
- 민감메모 저장은 키 준비, 저장 중, 성공, 실패 상태를 표시하고 실패 시 버튼이 멈춘 것처럼 보이지 않도록 수정했습니다.
- 설정의 점수 라벨 영역에서 `적극 지원 / 지원 고려 / 정보 추가 필요` 기준 점수를 조정할 수 있으며, 통계 점수 구간도 같은 기준을 사용합니다.
- 좌측 사이드바의 설정, 계정 이메일, 로그아웃을 하단 영역으로 묶고 바닥에 붙지 않도록 여백을 조정했습니다.
