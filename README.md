...
- 어드민 접근 진단 개선: 접근 실패 시 `403 권한 없음` / `503 서버 설정 오류(SUPABASE_SERVICE_ROLE_KEY)` / `500 서버 오류`를 각각 구분해 표시합니다. `profiles` 쿼리 예외도 안전하게 처리합니다.
- 어드민 대시보드 UX: 상태 변경 버튼에 로딩 스피너 추가, 성공/실패 토스트 피드백 추가.
- 서비스명 교체: "Company Signal" → "CompanyRadar" (이용약관·개인정보처리방침·환불정책·관리자 이메일 템플릿).
- 법률 페이지 이메일: `NEXT_PUBLIC_SUPPORT_EMAIL` 환경변수 사용, 미설정 시 "설정 화면 내 문의 양식"으로 표시.
