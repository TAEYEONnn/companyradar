...
- 드로어의 `확인 / 리서치 / 검증` 날짜 버튼과 `검증 완료` CTA를 제거하고, `공고 확인했어요` 단일 액션으로 정리했습니다.
- `공고 확인했어요`를 누르면 마지막 공고 확인일과 검증 이력이 오늘 날짜로 갱신되고 전역 토스트가 표시됩니다.
- 좌측 사이드바의 월 지원 목표는 작고 조용한 진행 표시로 낮춰 회사 탐색을 방해하지 않게 했습니다.
- 설정 화면은 `계정 → 내 직군 및 평가 기준 → 데이터`를 먼저 보여주고, 문의/환불/탈퇴는 하단의 덜 강조된 영역으로 이동했습니다.
- 서비스 문의, 환불, 탈퇴 접수 결과는 설정 카드 안 고정 문구 대신 전역 토스트로 표시합니다.
- 환불 요청은 승인된 결제 이력이 있는 계정만 접수할 수 있도록 클라이언트와 서버 API에서 모두 제한합니다.
- 민감메모는 암호화 키 로딩/준비/실패 상태를 구분하고, 키 준비 실패 시 재시도 UI를 제공합니다.
