...
- **AI 공고 분석 무료 1회 안내**: 온보딩 2단계와 Inbox 상단에 "AI 공고 분석 무료 1회" 배지 표시. 사용 완료 후 안내 문구로 전환.
- **AI 분석 버튼 상태 구분**: 분석 가능("AI 분석 · 무료 1회") / 분석 중("분석 중") / 완료("AI 분석 완료") 세 가지 상태로 표시.
- **AI 분석 초안 안내**: 파싱 성공(parsed/partial) 후 "AI 정리는 초안이에요. 저장 전 회사명, 공고명, 마감일을 직접 확인해주세요." 안내 표시.
- **시작 가이드 카드**: 회사가 0개일 때 핵심 4단계 흐름(공고 저장 → 신호 정리 → 지원 상태 관리 → 오늘 할 일)을 카드 형태로 안내. AI/직접 추가 CTA 포함.
