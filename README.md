...
- README 환경변수 섹션에서 운영자 이메일 예시 값을 제거했습니다. `.env.local`에 직접 설정하세요.
- 드로어 회사조사 탭의 "걱정되는 점" 중복을 수정했습니다. 리스크 항목 체크리스트 헤더가 "리스크 체크리스트"로 변경됩니다.
- 로컬 환경(devTools 활성 시, 미로그인 상태)에서도 민감메모를 테스트할 수 있습니다. 로컬 암호화 키는 `dev_local_user` 기준으로 저장됩니다.
- Toolbar 샘플 버튼 레이블을 "예시 데이터"로 변경하고, 확인 다이얼로그에 현재 직군의 예시 회사명(예: PM/PO 직군 → 당근)을 표시합니다.
- 사이드바 하단에 "운영자 대시보드" 링크가 추가됩니다. devTools 활성 상태(localhost 또는 `AI_ALLOWED_EMAILS` 계정)일 때만 표시됩니다.
