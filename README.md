...
- **비밀번호 재설정 경로 안정화**: 재설정 메일을 `/auth/callback`을 통해 라우팅하도록 변경해 Supabase 허용 redirect URL 설정 누락 문제를 우회. reset-password 페이지에서 PKCE 코드 교환 실패 시(다른 기기/브라우저로 링크 열기) 기존 세션으로 fallback하도록 보강.
